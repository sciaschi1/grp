...
function GRolePlay.Gamemode:GetPayday(ply)
	return ply.Payday
end

function GRolePlay.Gamemode:Payday(ply)
	for _,ply in pairs( player.GetAll() ) do
		GRolePlay.DB:Query("UPDATE `grp_player` SET money = '".. (GRolePlay.Player.Money + GRolePlay.Player.Payday) .."' WHERE UID ="..SQLStr(ply:UniqueID())..";")
	end
end
